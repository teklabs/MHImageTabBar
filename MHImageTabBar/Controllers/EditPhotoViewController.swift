...
class EditPhotoViewController: UIViewController, UITextFieldDelegate, UIScrollViewDelegate {
    ...
    override func loadView() {
        ...
        var footerRect: CGRect = PhotoDetailsFooterView.rectForView()
        footerRect.origin.y = photoImageView.frame.origin.y + photoImageView.frame.size.height

        let footerView = PhotoDetailsFooterView(frame: footerRect)
    ...
    func doneButtonAction(sender: AnyObject) {
        ...
        if (trimmedComment.characters.count != 0) {
            userInfo = [kEditPhotoViewControllerUserInfoCommentKey: trimmedComment]
        }
        ...
        let photo = PFObject(className: kPhotoClassKey)
        ...
        photo.saveInBackgroundWithBlock { (succeeded, error) in
            if succeeded {
                ...
                /*if let userInfo = userInfo {
                    let commentText = userInfo[kEditPhotoViewControllerUserInfoCommentKey]
