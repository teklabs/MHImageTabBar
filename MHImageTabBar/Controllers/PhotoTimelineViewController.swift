...
class PhotoTimelineViewController: PFQueryTableViewController, PhotoHeaderViewDelegate {
    ...
    deinit {
        ...
        defaultNotificationCenter.removeObserver(self, name: TabBarControllerDidFinishEditingPhotoNotification, object: nil)
        defaultNotificationCenter.removeObserver(self, name: UtilityUserFollowingChangedNotification, object: nil)
        defaultNotificationCenter.removeObserver(self, name: PhotoDetailsViewControllerUserLikedUnlikedPhotoNotification, object: nil)
        defaultNotificationCenter.removeObserver(self, name: UtilityUserLikedUnlikedPhotoCallbackFinishedNotification, object: nil)
        defaultNotificationCenter.removeObserver(self, name: PhotoDetailsViewControllerUserCommentedOnPhotoNotification, object: nil)
        defaultNotificationCenter.removeObserver(self, name: PhotoDetailsViewControllerUserDeletedPhotoNotification, object: nil)
    }
    ...
    override init(style: UITableViewStyle, className: String?) {
        ...
        super.init(style: style, className: kPhotoClassKey)
        
        // The className to query on
        self.parseClassName = kPhotoClassKey
...
*/
    ...
    override func queryForTable() -> PFQuery {
        ...
        autoFollowUsersQuery!.whereKey(kUserAutoFollowKey, equalTo: true)
    ...
    // MARK:- PhotoTimelineViewController
...
//        let likeCount: Int = numberFormatter.numberFromString(button.titleLabel!.text!)
        ...
        if liked {
            Utility.likePhotoInBackground(photo, block: { (succeeded, error) in
                // FIXME: nil??? same as the original AnyPic. Dead code?
                let actualHeaderView: PhotoHeaderView? = self.tableView(self.tableView, viewForHeaderInSection: button.tag) as? PhotoHeaderView
            ...
            Utility.unlikePhotoInBackground(photo, block: { (succeeded, error) in
                // FIXME: nil??? same as the original AnyPic. Dead code?
                let actualHeaderView: PhotoHeaderView? = self.tableView(self.tableView, viewForHeaderInSection: button.tag) as? PhotoHeaderView
        ...
        if attributesForPhoto != nil {
            ...
            synchronized(self) {
                ...
                if outstandingSectionHeaderQueryStatus == nil {
                    let query: PFQuery = Utility.queryForActivitiesOnPhoto(object!, cachePolicy: PFCachePolicy.NetworkOnly)
