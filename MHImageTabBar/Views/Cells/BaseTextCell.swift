...
class BaseTextCell: PFTableViewCell {
    var horizontalTextSpace: Int = 0
    var _delegate: BaseTextCellDelegate?
...
//    }
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        horizontalTextSpace = Int(BaseTextCell.horizontalTextSpaceForInsetWidth(cellInsetWidth))
...
// FIXME: Why nameSize is used before as the argument at the same time????        let nameSize: CGSize = name.boundingRectWithSize(nameSize,
        ...
        let paddedString: String = BaseTextCell.padString(content, withFont: UIFont.systemFontOfSize(13), toWidth: nameSize.width)
        let horizontalTextSpace: CGFloat = BaseTextCell.horizontalTextSpaceForInsetWidth(cellInset)
    ...
    func setContentText(contentString: String) {
        ...
        if self.user != nil {
            let nameSize: CGSize = self.nameButton!.titleLabel!.text!.boundingRectWithSize(CGSizeMake(nameMaxWidth, CGFloat.max),
                                                         ...
                                                         attributes: [NSFontAttributeName: UIFont.boldSystemFontOfSize(13.0)],
                                                            context: nil).size
            let paddedString: String = BaseTextCell.padString(contentString, withFont: UIFont.systemFontOfSize(13), toWidth: nameSize.width)
...
@objc protocol BaseTextCellDelegate: NSObjectProtocol {
    /*!
