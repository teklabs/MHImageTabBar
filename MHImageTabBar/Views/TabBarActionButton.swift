...
//  Pods
//
//  Created by teklabsco on 12/18/15.
//
//

import UIKit

public typealias TabBarActionButtonAction = (TabBarActionButton) -> Void

public class TabBarActionButton: NSObject {
    
    /// The action the button should perform when tapped
    public var action: TabBarActionButtonAction?
    ...
    internal var items: [TabBarActionButtonItem]? {
        willSet {
    ...
    public init(attachedToView view: UIView, items: [TabBarActionButtonItem]?) {
        super.init()
        ...
        self.floatButton.layer.shadowOpacity = 0
        self.floatButton.layer.shadowRadius = 0
    ...
    */
    private func installConstraints() {
        ...
        let trailingSpacing = NSLayoutConstraint.constraintsWithVisualFormat("V:[floatButton]-2-|", options: NSLayoutFormatOptions.AlignAllCenterX, metrics: nil, views: views)
        let bottomSpacing = NSLayoutConstraint.constraintsWithVisualFormat("H:[floatButton]-2-|", options: NSLayoutFormatOptions.AlignAllCenterX, metrics: nil, views: views)
    ...
    //MARK: - Custom Methods
    /**
    Presents or hides all the TabBarActionButton's actions
    */
    ...
    /**
    Defines the position of all the TabBarActionButton's actions
    */
    private func placeButtonItems() {
        if let optionalItems = self.items {
            for item in optionalItems {
                item.view.center = CGPoint(x: self.floatButton.center.x - 283, y: self.floatButton.center.y)
    ...
    /**
    Presents or hides all the TabBarActionButton's actions and changes the *active* state
    */
    private func toggle() {
        self.animateMenu()
        self.showBlur()
